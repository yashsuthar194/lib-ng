  ...
  template: `
    <div class="example-page">
      ...
      <section class="example-section">
        ...
        <button class="btn btn--primary" (click)="openAlert()">Show Alert</button>
        @if (alertResult()) {
      ...
      <section class="example-section">
        ...
        <button class="btn btn--danger" (click)="openConfirm()">Delete Item</button>
        @if (confirmResult()) {
          <p class="result">
            Result: {{ confirmResult()!.result ? 'Confirmed' : 'Cancelled' }} (via
      ...
      <section class="example-section">
        ...
        <button class="btn btn--secondary" (click)="openForm()">Edit Profile</button>
        @if (formResult()) {
          <p class="result">
...
export class BasicModalExampleComponent {
  ...
  readonly formResult = signal<ModalCloseResult<{
    name: string;
    email: string;
    saved: boolean;
  } | null> | null>(null);
  ...
  openForm(): void {
    ...
    modalRef.afterClosed$.subscribe(result => {
      this.formResult.set(
        result as ModalCloseResult<{ name: string; email: string; saved: boolean } | null>
      );
    });
  }
}
