...
export class ModalService {
   ...
   */
  open<C, D = unknown, R = unknown>(component: Type<C>, config?: ModalConfig<D>): ModalRef<D, R> {
    ...
    const zIndex = mergedConfig.zIndex ?? MODAL_Z_INDEX_BASE + stackIndex * MODAL_Z_INDEX_INCREMENT;

    // Create ModalRef
    const modalRef = new ModalRef<D, R>(mergedConfig.data as D, mergedConfig);
    ...
    containerRef.instance.initialize(component, modalRef, mergedConfig, zIndex);
  ...
  private removeFromStack(id: string): void {
    this._modalStack.update(stack => stack.filter(item => item.id !== id));
    this._openModals.update(modals => modals.filter(modal => modal.id !== id));
  }
}
