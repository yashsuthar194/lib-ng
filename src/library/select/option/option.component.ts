    ...
    `
      ...
      :host-context([data-theme-mode='dark']).lib-option--selected {
        background: color-mix(
          in srgb,
          var(--lib-color-primary-500) 15%,
          var(--lib-color-neutral-900)
        );
...
export class OptionComponent<T = unknown> {
  ...
  /** Unique ID for this option (for aria-activedescendant) */
  readonly id = `lib-option-${++OptionComponent.instanceCounter}`;
  ...
  matchesSearch(query: string): boolean {
    ...
    return this.getSearchTerms().some(term => term.toLowerCase().includes(lowerQuery));
  }
